    ...
    useEffect(() => {
        ...
        const renderDiagram = async (div: HTMLElement) => {
            try {
                ...
                // Create a wrapper with theme-aware classes
                const svgWrapper = document.createElement('div');
                svgWrapper.className = 'mermaid-svg-wrapper w-full overflow-auto bg-white dark:bg-dark-background transition-colors duration-200';
        ...
        `;
                ...
                // Ensure SVG has proper styling for both themes
                const svgElement = svgWrapper.querySelector('svg');
                if (svgElement) {
                    // Remove any hardcoded background color
                    svgElement.style.removeProperty('background-color');

                    // Add theme-aware classes to SVG
                    svgElement.classList.add('transition-colors', 'duration-200');

                    // Ensure text elements are visible in both themes
                    const textElements = svgElement.querySelectorAll('text');
                    textElements.forEach(text => {
                        text.classList.add('fill-dark-text', 'dark:fill-white');
                        // Remove any hardcoded fill colors
                ...
                for (const div of Array.from(mermaidDivs)) {
                    try {
                        ...
                        if (!wrapper) {
                            ...
                            wrapper.className = 'mermaid-wrapper relative overflow-x-auto my-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-dark-background transition-colors duration-200';
